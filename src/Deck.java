...
public class Deck {
	...
	public void printPile(ArrayList<Card> pileTest) {
		if (pileTest.size() < 1) {
			System.out.println("Empty Deck");
		} else {
			for (int i = 0; i < pileTest.size(); i++) {
				System.out.println(pileTest.get(i).getName());
			}
		}
	}

	public void dealPile(ArrayList<Card> playPiles, ArrayList<Card> deck) {
		for (int i = 0; i < 20; i++) {
